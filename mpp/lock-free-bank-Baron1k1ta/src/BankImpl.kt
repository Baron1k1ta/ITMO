...
class BankImpl(override val accountsCount: Int) : Bank {
    ...
    private inner class TransferOp(val fromId: Int, val toId: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
            ...
            val first = acquire(firstId, this)
            if (first == null) {
                // Already completed by another helper.
                release(firstId, this)
                release(secondId, this)
                return
            }
            val second = acquire(secondId, this)
            if (second == null) {
                release(firstId, this)
                release(secondId, this)
                return
            ...
            if (!completed) {
                val from = if (fromId == firstId) first else second
                val to = if (toId == firstId) first else second

                val fromAmount = from.amount
                val toAmount = to.amount

                val err = when {
                    amount > fromAmount -> "Underflow"
                    toAmount + amount > MAX_AMOUNT -> "Overflow"
                    else -> null
                }

                errorMessage = err
                if (err == null) {
                    from.newAmount = fromAmount - amount
                    to.newAmount = toAmount + amount
                }
                completed = true // publish results
            }
