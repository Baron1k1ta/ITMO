...
import info.kgeorgiy.java.advanced.base.RunnableE;
...
public class SortedSetTest extends BaseSetTest {
    ...
    protected final Random random = new Random(5390845702394857203L);
    ...
    public void test05_constructorPerformance() {
        performance("constructor", this::performanceSet);
    }
    ...
    protected void performance(final String description, final RunnableE<IllegalArgumentException> runnable) {
        context.context(description + " performance", () -> {
            runnable.run();
            final long time = context.measureTime(runnable);
            Assertions.assertTrue(time < PERFORMANCE_TIME, description + " works too slow");
        });
    }

    protected void performance(final String description, final Consumer<SortedSet<Integer>> action) {
        performance(description, () -> action.accept(performanceSet()));
    }

    protected SortedSet<Integer> performanceSet() {
        return set(performanceList(PERFORMANCE_SIZE), Comparator.nullsFirst(Comparator.naturalOrder()));
    }

    protected List<Integer> performanceList(final int size) {
        return random.ints().limit(size).boxed().distinct().collect(Collectors.toList());
    }
    ...
    private static void testIterator(final SetPair<Integer, Collection<Integer>> pair) {
