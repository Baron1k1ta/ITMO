...
public class BTreeAdapt implements Storage {
    ...
    public int find(long key, Holder<Object> result) {
        totalAccessCount++;
        int found = root.find(key, result);

        
        if (found == OK && totalAccessCount > HOT_THRESHOLD) {
            adaptNodeAfterAccess(key);
        }
    ...
    public int remove(long key) {
        if (root.size == 1 && root.children[0] != null &&
                root.children[0].size == default_t - 1 &&
                root.children[1] != null && root.children[1].size == default_t - 1) {
    ...
    private void adaptNodeAfterAccess(long key) {
        
        Node node = root;
        while (node != null) {
            int position = lowerBound(node.keys, node.size, key);
            if (position < node.size && node.keys[position] == key) {
                
                if (node.parent != null && node.accessCount[position] > HOT_THRESHOLD) {
                    tryPromoteKey(node, position);
                }
                return;
            }
            node = (node.children[position] != null) ? node.children[position] : null;
        }
    }

    private void tryPromoteKey(Node node, int position) {
        Node parent = node.parent;
        if (parent == null) return;

        int parentPos = findChildPosition(parent, node);
        if (parentPos >= 0) {
            
            long tempKey = node.keys[position];
            Object tempValue = node.values[position];
            int tempAccess = node.accessCount[position];

            node.keys[position] = parent.keys[parentPos];
            node.values[position] = parent.values[parentPos];
            node.accessCount[position] = parent.accessCount[parentPos];

            parent.keys[parentPos] = tempKey;
            parent.values[parentPos] = tempValue;
            parent.accessCount[parentPos] = tempAccess;
        }
    }

    private int findChildPosition(Node parent, Node child) {
        for (int i = 0; i <= parent.size; i++) {
            if (parent.children[i] == child) {
                return (i < parent.size) ? i : i - 1;
            }
        }
        return -1;
    }
    ...
    private static class Node {
        ...
        public Node(int t, BTreeAdapt tree){
            this.t = t;
            this.tree = tree;
            parent = null;
            size = 0;
        ...
        public int insert(long key, Object value) {
            if (children[0] == null) {
                ...
                insertValue(accessCount, size, position, 0); 
                size++;
                tree.treeSize++;
                return OK;
            }
        ...
        private void split() {
            ...
            System.arraycopy(accessCount, t, brother.accessCount, 0, t - 1);
            if (children[0] != null) {
            ...
            if (parent == null) {
                ...
                insertValue(parent.accessCount, parent.size, pivot_position, pivot_access);
        ...
        public int remove(long key) {
            if (children[0] == null) {
                ...
                if (position < size && keys[position] == key) {
                    ...
                    removeValue(accessCount, size, position);
                    size--;
                    tree.treeSize--;
                    return OK;
                }
            ...
            if (position < size && keys[position] == key) {
                ...
                accessCount[position] = targetNode.accessCount[targetNode.size];
            ...
            if (size == t - 1) {
                ...
                if (brother.size == t - 1) {
                    ...
                    if (isBrotherRight) {
                        ...
                        insertValue(accessCount, size, size, parent.accessCount[my_index]);
                        ...
                        parent.accessCount[my_index] = removeValue(brother.accessCount, brother.size, 0);
                        ...
                        insertValue(accessCount, size, 0, parent.accessCount[my_index - 1]);
                        ...
                        parent.accessCount[my_index - 1] = removeValue(brother.accessCount, brother.size, brother.size - 1);
        ...
        private void merge(Node brother, int my_idx) {
            ...
            System.arraycopy(brother.accessCount, 0, accessCount, t, t - 1);
            ...
            del_access = removeValue(parent.accessCount, parent.size, my_idx);

            size = 2 * t - 1;
            keys[t - 1] = del_key;
            values[t - 1] = del_value;
            accessCount[t - 1] = del_access;
            parent.size--;
        }
