...
type (
	AuthorUseCase interface {
		...
		UpdateAuthor(ctx context.Context, id string, name string) error
	}

	BooksUseCase interface {
		RegisterBook(ctx context.Context, name string, authorID []string) (entity.Book, error)
		GetBook(ctx context.Context, bookID string) (entity.Book, error)
		UpdateBook(ctx context.Context, id string, name string, authorID []string) error
